...
const RenderResults = ({sortByTime, teams, loading, setLoading}) => {
    ...
    useEffect(() => {
        sortByTime.length && sortByTime.reverse().map((item, index) => {
            results.length !== sortByTime.length && setResults( prev => [...prev, index]);
                ...
                { sortByTime[index].map(item => (
                    <div key={`${index += 1}`} className='results-container'>
                        ...
                        { teams.filter(team => team.name === item.homeTeam)
                            .map(team => <img key={`${index} ${index += 3}`} width='45px' height='45px' src={team.badge} /> 
                        )}
                        { item.homeScore ? <p key={`${index} ${index += 4}`}>
                        ...
                        { teams.filter(team => team.name === item.awayTeam).map(team => <img key={`${index} ${index += 5}`} width='45px' height='45px' src={team.badge} /> ) }
                        <p key={`${index} ${index += 6}`}>
                            {item.awayTeam}
                        </p>
